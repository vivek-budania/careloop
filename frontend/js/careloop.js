 ...
 */
const CareLoop = {
  THREAD_KEY: 'careloop-patient-thread-v1',
  view: 'Today',
  historyTab: 'visits',
  selectedVisit: null,
  insuranceMode: 'hub',
  ...
  firstName() {
    return (this.thread.patient.name || 'Maya').split(' ')[0];
  },

  seedThread(mode) {
    const name = (App.user && App.user.name) || 'Maya Chen';
  ...
  renderLogin() {
    ...
    app.innerHTML = `<div class="login"><section class="login-story">${this.logo()}<h1>Your health.<br>Your story.<br><em>All together.</em></h1><p>A little less to keep track of.<br>A little more peace of mind.</p>${this.art()}<small>One connected journey. From your first visit to what’s next.</small></section><section class="login-form"><form id="login-form"><span class="eyebrow">A little clarity, every day</span><h2>Welcome to your care.</h2><p>Keep your visits, medicines, and next steps in one place.</p><label class="field">Username<input name="username" autocomplete="username" value="maya" required></label><label class="field">Password<input name="password" type="password" autocomplete="current-password" value="demo" required></label><div class="error" id="login-error" role="alert"></div><button class="btn" type="submit" name="mode" value="returning">I’m returning ${this.icon('arrow')}</button><button class="btn secondary" type="submit" name="mode" value="first">Start my first visit</button><div class="hint">Password for every account is <strong>demo</strong><br>maya · priya · advocate · demo</div><p style="text-align:center;margin:22px 0 0;font-size:10px">Interactive demo · Fictional patient data<br>Care drafts are always for clinician review.</p></form></section></div>`;
  ...
  today() {
    ...
    return `<section class="greeting"><div><div class="eyebrow">Thursday, September 24</div><h1>A little clarity, ${this.esc(first)}.</h1><p>Here’s where things stand — and what comes next.</p></div><div class="date">${this.icon('calendar')} Your personal care space</div></section><div class="grid"><div class="stack"><section class="card hero"><div class="eyebrow">${complete ? 'One step forward' : 'Your next step'}</div><h2>${complete ? 'Your visit, all in one place.' : j ? 'Let’s pick up where you left off.' : 'Let’s make your next visit easier.'}</h2><p>${complete ? 'Your summary and next steps are saved. Take your story with you to the next visit.' : 'A few details now. A clearer conversation with your doctor later.'}</p>${this.btn(complete ? 'View visit summary' : j ? 'Continue your visit' : 'Prepare for your visit', complete ? 'latest-visit' : 'start')}${this.art()}</section><section class="card"><div class="section-heading"><h2>${j?.slot ? 'Your requested appointment' : 'Your upcoming visit'}</h2>${this.tag(j?.slot ? 'Request saved' : 'Demo appointment', 'peach')}</div><div class="appointment"><div class="day-box"><small>SEP</small><strong>24</strong></div><div><small>PRIMARY CARE · FOLLOW-UP</small><h3>${this.esc(j?.doctor || 'Dr. Priya Shah')}</h3><small>${this.esc(j?.clinic || 'Mission Family Clinic')}</small></div>${this.tag(c?.status === 'active' ? 'In network · mock' : 'Confirm network', c?.status === 'active' ? '' : 'peach')}</div><div class="rule"></div><div class="details"><span>${this.icon('clock')}${this.esc(j?.slot || '10:30 AM')} · 30 min</span><span>${this.icon('pin')}San Francisco, CA</span></div></section><section class="card"><div class="section-heading"><h2>A few things for today</h2><small>Small steps count.</small></div><div class="task-row"><div class="tile-icon peach">${this.icon('pill')}</div><div><h3>Your evening medicine</h3><small>Metformin · 8:00 PM · ${this.esc(this.thread.doses.evening)}</small></div>${this.link('View', 'medicines')}</div><div class="task-row"><div class="tile-icon lilac">${this.icon('test')}</div><div><h3>HbA1c blood test</h3><small>${j?.reviewed ? 'Mock order ready · no result yet' : 'Waiting for clinic review'}</small></div>${this.link('Details', 'tests')}</div><div class="task-row"><div class="tile-icon">${this.icon('file')}</div><div><h3>Your story, ready for the clinic</h3><small>Visits, medicines, and coverage together</small></div>${this.link('Prepare', 'packet')}</div></section></div><div class="stack"><section class="card"><div class="progress-top"><h2>Your care journey</h2>${this.tag('In progress', 'gray')}</div><ol class="timeline"><li><span class="point">${c ? '✓' : '1'}</span><div><h3>${c ? 'Insurance added' : 'Add insurance, if you like'}</h3><p>${c ? `${this.esc(c.payer)} · ${this.esc(c.status)} (mock)` : 'Optional. You can still start a visit.'}</p></div></li><li><span class="point ${complete ? '' : 'now'}">${complete ? '✓' : '2'}</span><div><h3>${complete ? 'Your visit is saved' : 'Prepare for your visit'}</h3><p>${complete ? 'Summary available in your history' : 'Share what’s on your mind.'}</p>${this.tag(complete ? 'Saved' : 'Your next step', complete ? '' : 'peach')}</div></li><li><span class="point ${complete ? 'now' : 'empty'}">3</span><div><h3>Visit &amp; care plan</h3><p>${complete ? 'Clinic reviews your next steps.' : 'A clear summary. A plan to review.'}</p></div></li><li><span class="point empty">4</span><div><h3>Keep your care moving</h3><p>Tests, medicines, and follow-ups.</p></div></li></ol></section><section class="card insurance-mini"><div class="row"><span class="eyebrow">Your coverage</span>${this.icon('shield')}</div><h3>${c ? `${this.esc(c.payer)} · ${this.esc(c.plan || '')}` : 'No insurance on file'}</h3><p>${c ? 'Mock coverage snapshot · estimates only' : 'Add a plan for estimated costs.'}</p>${c ? `<div class="row"><div><span class="money">${c.status === 'active' ? this.money(c.copay) : '—'}</span><small>&nbsp; est. PCP copay</small></div></div><div class="rule"></div>` : ''}${this.link('View insurance', 'insurance')}</section></div></div>`;
  },

  setup() {
    let content = '';
    if (this.insuranceMode === 'hub') {
      content = `<h2>A good place to start.</h2><p>Add your insurance to see estimated costs and mock in-network clinics. You can also skip this for now.</p><div class="split"><button class="card" style="text-align:left" data-action="sample-card" type="button">${this.icon('camera')}<h3 class="mt">Try a sample card</h3><p style="font-size:12px;margin-top:8px">Review prefilled demo details.<br>No camera or OCR required.</p></button><button class="card" style="text-align:left" data-action="manual-card" type="button">${this.icon('file')}<h3 class="mt">Enter plan details</h3><p style="font-size:12px;margin-top:8px">Choose your insurance company.<br>The rest is optional.</p></button></div><div class="actions">${this.link('Skip for now', 'skip-insurance')}</div>`;
    } else {
      const p = this.coverageSnap.profile || {};
      const sample = this.insuranceMode === 'sample';
      const selected = p.payer_name || (sample ? 'Mock Payer' : '');
      const options = (this.payers.length ? this.payers : [{ name: 'Mock Payer', plan_type: 'PPO' }])
        .map((row) => `<option value="${this.esc(row.name)}" ${selected === row.name ? 'selected' : ''}>${this.esc(row.name)} (${this.esc(row.plan_type)})</option>`)
        .join('');
      content = `<h2>${sample ? 'Review your sample card.' : 'A few plan details.'}</h2><p>${sample ? 'These fields come from a fixture, not a scan. You can edit them before saving.' : 'Only the insurance company is required. Use fictional details for this demo.'}</p><form id="insurance-form"><label class="field">Insurance company<select name="payer" required><option value="">Select an insurer</option>${options}</select></label><label class="field">Member name (optional)<input name="member_name" value="${this.esc(p.member_name || this.thread.patient.name)}"></label><div class="split"><label class="field">Member ID (optional)<input name="member" value="${this.esc(p.member_id || '')}"></label><label class="field">Group number (optional)<input name="group" value="${this.esc(p.group_number || '')}"></label></div><label class="field">ZIP code<input name="zip" value="${this.esc(p.zip || this.thread.patient.zip || '94110')}" pattern="[0-9]{5}" maxlength="5"></label><div class="notice">Demo eligibility only. This does not verify real coverage or decide benefits. Estimates are not a bill.</div><div class="actions">${this.btn('Back', 'insurance-hub', 'secondary')}<button class="btn" type="submit">Save &amp; review coverage ${this.icon('arrow')}</button></div></form>`;
    }
  ...
  async loadNetwork() {
    ...
    try {
      const payload = await API.searchNetwork('pcp', zip);
  ...
  journey() {
    ...
    switch (j.step) {
      ...
      case 2: {
        ...
        const rows = list.length
          ? list.map((doc) => {
            ...
            return `<div class="doctor"><div class="avatar">${this.esc(this.initials(doc.name))}</div><div><h3>${this.esc(doc.name)}</h3><p>${this.esc(doc.specialty_label)} · ${doc.miles} mi<br>${this.esc(doc.address)}</p>${this.tag(inNet ? 'In network · mock' : 'Confirm network', inNet ? '' : 'peach')}</div>${this.btn(selected ? 'Selected ✓' : 'Choose', 'choose-doctor', selected ? '' : 'secondary', `data-npi="${this.esc(doc.npi)}"`)}</div>`;
          }).join('')
          : `<p>Loading fixture clinicians near ${this.esc(zip)}…</p>`;
        body = `<h2>A familiar face. Or a fresh start.</h2><p>Mock directory near ${this.esc(zip)}, filtered by distance against Dave’s fixture list. The clinic still confirms availability and network status.</p>${rows}`;
        break;
      }
      case 3:
      ...
      case 4:
        ...
        break;
      case 5:
        body = `<div class="row" style="justify-content:space-between"><h2>The conversation, captured.</h2>${this.tag('Sample transcript', 'gray')}</div><p>A seeded conversation for this demo. A clinician reviews the summary.</p><div class="transcript"><p><strong>MAYA · 00:08</strong>“I’ve been feeling more tired and thirsty. I’m still taking my metformin twice a day.”</p><p><strong>DR. SHAH · 00:24</strong>“Let’s review how things have been going and discuss an HbA1c test.”</p><p><strong>MAYA · 00:41</strong>“I’d like to understand what happens next, especially with my insurance.”</p><p><strong>DR. SHAH · 01:02</strong>“We can discuss an add-on medicine after reviewing your results. It may need prior authorization — that’s separate from any later claim.”</p></div>`;
        break;
      case 6:
  ...
  bindForms() {
    ...
    if (profile) {
      profile.addEventListener('submit', (e) => {
        ...
        this.saveThread({
          patient: {
            name: String(d.get('name') || '').trim() || 'Maya Chen',
            email: d.get('email'),
            zip: d.get('zip'),
          },
        });
  ...
  async saveInsuranceForm(form) {
    ...
    try {
      ...
      this.toast('Plan saved. Showing mock coverage for review.');
