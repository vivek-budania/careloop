...
const CareLoop = {
  ...
  costBody() {
    ...
    if (!demo && !this.transcriptText()) {
      return `<h2>Your estimated costs.</h2><p>Cost estimates come from your visit transcript unless you pick Demo 1, 2, or 3.</p><div class="notice">No transcript yet, so there is no cost estimate to show. Demo conversations still use the sample fee schedule.</div>${claim}`;
    }
    if (!estimate) {
      return `<h2>Your estimated costs.</h2><p>${demo ? 'Building your cost estimate from the selected demo conversation…' : 'Building your cost estimate from this visit transcript…'}</p><div class="notice">${demo ? 'Demo sample amounts based on your saved plan. Estimate only — not a bill or a coverage decision.' : 'If this stays empty, the transcript did not have enough service detail. That’s alright for now.'}</div>${claim}`;
    }
    const lines = estimate.likely_visits || [];
    if (!lines.length && !(estimate.medicines || []).length) {
      return `<h2>Your estimated costs.</h2><p>${demo ? 'The selected demo did not return priced services.' : 'Nothing billable could be pulled from this transcript yet.'}</p><div class="notice">${this.esc((estimate.warnings && estimate.warnings[0]) || 'No demo boilerplate is shown for a live transcript.')}</div>${claim}`;
    }
