...
**Database (hosted Supabase, not wired to coverage/login beyond `profiles`):** [`docs/database/`](docs/database/README.md) (`profiles`, `visits`, `insurance`, `medicines`, `tests`, `intakes`, `claims`, `logins`). Matching SQL: [`supabase/migrations/`](supabase/migrations/). `logins` is a sign-in event log (no password column); password hashes stay in Auth.
