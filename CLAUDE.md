...
**Database (hosted Supabase, not wired to coverage/login beyond `profiles`):** [`docs/database/`](docs/database/README.md) (`profiles`, `visits`, `insurance`, `medicines`, `tests`, `intakes`, `claims`). Matching SQL: [`supabase/migrations/`](supabase/migrations/). No `login` table.
