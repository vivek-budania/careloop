...
Signup and login use **Auth + `public.profiles`** for credentials. `public.logins` is a sign-in **event** log (no password column) — not wired yet. See [`docs/database/`](../docs/database/README.md).

## Layout

| Path | Role |
|------|------|
| [`migrations/`](migrations/) | Timestamped `.sql` files for hosted tables (`profiles`, `visits`, `insurance`, `medicines`, `tests`, `intakes`, `claims`, `logins`). |
...
Vivek already created `visits`, `insurance`, `intakes`, and `claims` in the hosted SQL editor / Table Editor. `profiles` was created in the dashboard UI (login lookup). `medicines`, `tests`, and `logins` are **new** in this folder — apply those files on hosted CareLoop after `visits` / `profiles`. These files are **idempotent** (`CREATE TABLE IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`, `DROP POLICY IF EXISTS` then recreate) so:
...
Do not treat applying these files as a data wipe. They do not insert Jane Doe, coverage rows, visits, medicines, tests, intakes, claims, or login events.
...
3. Seed Auth + a `profiles` row separately (demo: username `jane`, email `jane@careloop.local`). There is **no credentials table**. `logins` is an event log only.
...
`20260919108000_create_logins.sql` is an append-only sign-in event log. No password column. The app does not write it yet.
...
- Wiring coverage, intakes, medicines, tests, claims, or logins APIs, or changing login.
- PA/appeal letters, history packet, transcripts (not tables).
- Card images, API keys, password hashes, or file bytes as first-class columns (`tests.document_filename` is a filename only).
