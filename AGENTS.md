...
**Sreekar Stream C (visit-day from Upcoming visits):** booking steps 1–3 end at **Save request**, which confirms the appointment. Step 2 searches the mock directory by **ZIP + specialty from the visit reason** (nearby ≤ 40 miles, then farther alternatives). Open/upcoming visits can be deleted. Opening an upcoming visit starts visit-day: check-in → record / upload / **Demo 1, Demo 2, Demo 3** → summary → costs → plan. Demo 1 psoriasis/Skyrizi, Demo 2 lumbar MRI, Demo 3 chronic-migraine Botox (`GET /api/careloop/scribe/demos`). Finishing the plan opens a follow-up summary of medicines to take/buy and tests to complete, with **Update Prescriptions and Test records**. Audio goes to `POST /api/careloop/scribe/transcribe` (2-minute cap). A selected demo drafts SOAP with `use_seeded: true` and `demo_id`; live audio uses `use_seeded: false`. Step 6 calls `POST /api/careloop/scribe/summarize` (Sumy). On Vercel, NLTK corpora go to `/tmp/nltk_data`. `frontend/js/scribe.js` is still not loaded. The logged-in patient remains Jane Doe.
